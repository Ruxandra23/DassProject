...
def tickets():
    if not session.get("user_id"):
        return redirect(url_for("login"))
    # v1 vulnerabil: returneaza toate ticketele din sistem, nu doar ale userului curent
    # orice utilizator autentificat poate vedea ticketele celorlalti (IDOR)
    all_tickets = Ticket.query.all()
    _audit("VIEW_TICKETS", "ticket", user_id=session.get("user_id"))
...
def ticket_detail(ticket_id):
    if not session.get("user_id"):
        return redirect(url_for("login"))
    # v1 vulnerabil: nu se verifica daca ticketul apartine userului curent
    # oricine poate accesa orice ticket daca ghiceste ID-ul (IDOR)
    ticket = Ticket.query.get(ticket_id)
    if not ticket:
        flash("Ticket inexistent.", "error")
        return redirect(url_for("tickets"))
    _audit("VIEW_TICKET", "ticket", resource_id=str(ticket.id), user_id=session.get("user_id"))
