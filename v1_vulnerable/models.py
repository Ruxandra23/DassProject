...
class AuditLog(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, ForeignKey("users.id"), nullable=True) #poate lipsi pentru ca unele actiuni se intampla fara un user logat (de ex cineva care incearca sa se conecteze cu un email inexistent)
    action = db.Column(db.String(64), nullable=False)
    resource = db.Column(db.String(64), nullable=False)
    resource_id = db.Column(db.String(64), nullable=True) #pt login nu exista un ID specific
    timestamp = db.Column(db.DateTime, default=datetime.utcnow)
    ip_address = db.Column(db.String(45), nullable=True)
    #unele actiuni nu vin dintr un browser, deci nu au un IP 