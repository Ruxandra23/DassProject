...
from models import db, User, UserRole, AuditLog, Ticket
...
def login():
    ...
    db.session.commit()
    session.clear()          # rotatie sesiune la login -- previne session fixation
...
def tickets():
    if not session.get("user_id"):
        return redirect(url_for("login"))
    # v2 securizat: filtram dupa owner_id, userul vede doar ticketele lui
    user_tickets = Ticket.query.filter_by(owner_id=session["user_id"]).all()
    _audit("VIEW_TICKETS", "ticket", user_id=session["user_id"])
...
def ticket_detail(ticket_id):
    if not session.get("user_id"):
        return redirect(url_for("login"))
    # v2 securizat: cautam dupa id SI owner_id in acelasi timp
    # daca ticketul nu apartine userului curent, accesul este refuzat
    ticket = Ticket.query.filter_by(id=ticket_id, owner_id=session["user_id"]).first()
    if not ticket:
        flash("Acces interzis sau ticket inexistent.", "error")
        return redirect(url_for("tickets"))
    _audit("VIEW_TICKET", "ticket", resource_id=str(ticket.id), user_id=session["user_id"])
